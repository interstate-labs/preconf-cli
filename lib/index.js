...
async function getAvailableProposerDuties(network) {
  ...
  // Use our active validators system instead of static validators.json
  let validators = [];
  try {
    // Try to get active validators from our monitoring system
    const gatewayUrl = process.env.ROUTER_URL || config.ROUTER_URL || 'http://localhost:8000';
    const response = await axios.get(`${gatewayUrl}/api/validators/active`);
    if (response.data && Array.isArray(response.data)) {
      validators = response.data.map(validator => validator.pubkey);
      console.log(`Found ${validators.length} active validators from monitoring system`);
    }
  } catch (error) {
    console.warn('Failed to get active validators from monitoring system, falling back to static validators.json');
    // Fallback to the static validators.json file
...
// Import the active validators functions
export { recordValidatorActivity, getActiveValidators, getInactiveValidators, getValidatorStatus } from './active-validators.js';
